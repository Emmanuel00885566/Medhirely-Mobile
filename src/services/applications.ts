...
export const applicationsService = {
  ...
  withdrawApplication: async (workerId: string) => {
  try {
    const response = await api.patch(`/applications/withdrawApplication/${workerId}`);
    console.log('Withdraw response:', response.data);
    return response.data;
  } catch (error: any) {
    console.log('WITHDRAW APPLICATION ERROR:', error?.response?.data);
    throw error;
  }
},

  // ✅ Get application by ID (no endpoint yet)
